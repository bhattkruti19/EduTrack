...
MODEL_PATH = Path(__file__).resolve().parent.parent / "ml" / "cgpa_model.pkl"
DEFAULT_MODEL = {
    "intercept": 0.75,
...
def _predict(model, features):
    ...
    if isinstance(model, dict):
        ...
        weighted_sum = sum(float(weight) * float(value) for weight, value in zip(weights, features))
        return intercept + weighted_sum
...
def predict_cgpa():
    """API: Predict CGPA using saved pickle model based on performance inputs."""
    try:
